...
def main() -> None:
    ...
    findings["capture_rate"] = {
        ...
        "note": "Price a solar generator receives on the wholesale market, against "
                "the flat baseload price of the same year. Below 1.00 means solar "
                "generates when power is cheap.",
        "by_year": capture,
    }
    ...
    def run(location: str, profile: profiles.LoadProfile,
        ...
        # An inverter can be curtailed. Exporting into a negative price to earn a
        # negative revenue is a modelling artefact, not a business decision.
        export_revenue = float((exported * np.maximum(spot, 0)).sum())
        curtailed = float(exported[spot < 0].sum())
    ...
    for profile in profiles.PROFILES:
        ...
        for ratio in SIZING_RATIOS:
            ...
            row = {
                ...
                "curtailed_mwh": round(result["curtailed_kwh"] / 1000, 2),
    ...
    # The tables say what the overlap is worth; this says what it looks like, which
    # is the version that survives a meeting.
