...
What the model does, what it assumes, and where the assumptions matter.
...
**Revenue in negative-price hours is floored at zero.** Exporting into a negative
price to earn negative revenue is a modelling artefact; an operator would switch the
inverter off. Nothing is physically curtailed in the model — the energy is still
counted as generated and exported, and the self-consumption rate is unaffected. Only
the revenue on those hours is set to zero, and the volume concerned is reported as
`zero_revenue_export_mwh` so its size can be checked.
