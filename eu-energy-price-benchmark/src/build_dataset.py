...
def tidy_prices(frame: pd.DataFrame, band_names: dict[str, str]) -> pd.DataFrame:
    ...
    # Even with the unit pinned in the query, assert one price per country, band and
    # period. Duplicates raise no error on their own; they just shift every average.
